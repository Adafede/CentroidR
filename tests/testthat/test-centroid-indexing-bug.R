...
# ============================================================================
# TEST 6: Split dense profile peaks that do not return to baseline
# ============================================================================

test_that("BUG REPRODUCTION: dense profile trace splits into distinct peaks", {
  mz_trace <- c(
    164.0338839993696,
    164.0357619330662,
    164.03763986676282,
  ...
  full_centroid <- stats::weighted.mean(mz_trace, int_trace^3)
  .split_peak_group <- getFromNamespace(".split_peak_group", "CentroidR")
  segments <- .split_peak_group(seq_along(int_trace), int_trace)
  centroids <- vapply(
    segments,
    function(i) stats::weighted.mean(mz_trace[i], int_trace[i]^3),
    numeric(1)
  )

  expect_true(
    length(segments) >= 2,
    info = "Dense trace should split into peaks"
  )
  expect_true(
    any(abs(centroids - 164.0526638) < 0.001),
    info = "First peak centroid stays on m/z 164.05266"
  )
  expect_true(
    any(abs(centroids - 164.1071314) < 0.001),
    info = "Second peak centroid stays on m/z 164.10713"
  )
  expect_true(
    abs(full_centroid - 164.0526638) > 0.01,
    info = "Single broad centroid would drift away from the target peak"
  )
})
